...
function applicationView(document) {
  ...
  return {
    ...
    submittedByRole: document.submittedByRole || "member",
    submittedByMemberId: String(document.submittedByMemberId || document.memberId || ""),
...
async function adminApplications(request, response, url, context, actor, applicationId = "") {
  ...
  if (request.method === "GET") {
    const filter = { organisationKey, eventKey: NEXT_HUMAN_EVENT_KEY, status: { $ne: "draft" } };
...
async function memberOverview(response, context, actor) {
  ...
  const [event, application, bookings, reservations] = await Promise.all([
    ...
    db.collection("nextHumanApplications").findOne(
...
async function memberApplication(request, response, context, actor) {
  ...
  const questions = [...(event.pathways?.[validation.value.pathway]?.questions || [])];
  const update = {
    ...validation.value,
    questions,
    status,
    ...
    submittedByMemberId: actor._id,
    submittedByRole: ["administrator", "super_administrator"].includes(actor.role) ? "administrator" : "member",
    updatedAt: now,
    ...(submit ? { submittedAt: now } : {}),
  };
  const applicationCollection = db.collection("nextHumanApplications");
  let document;
  if (submit) {
    const submissionReference = `NHA-${now.getUTCFullYear()}-${randomUUID().slice(0, 8).toUpperCase()}`;
    const draft = await applicationCollection.findOneAndUpdate(
      { organisationKey, eventKey: NEXT_HUMAN_EVENT_KEY, memberId: actor._id, status: "draft" },
      { $set: { ...update, submissionReference } },
    ...
    if (draft) {
      ...
      await applicationCollection.insertOne(document);
    }
  } else {
    document = await applicationCollection.findOneAndUpdate(
      { organisationKey, eventKey: NEXT_HUMAN_EVENT_KEY, memberId: actor._id, status: "draft" },
      { $set: update, $setOnInsert: { organisationKey, eventKey: NEXT_HUMAN_EVENT_KEY, memberId: actor._id, createdAt: now } },
      { sort: { updatedAt: -1 }, upsert: true, returnDocument: "after" },
    );
  }
