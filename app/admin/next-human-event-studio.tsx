...
type Application = { id: string; memberNumber: string; fullName: string; email: string; mobile: string; dateOfBirth: string; pathway: "challenge" | "fellowship"; answers: string[]; status: string; score: number; internalNote: string; submittedAt: string | null };
...
type Tab = "command" | "programme" | "questions" | "applications" | "seating" | "bookings" | "founding";
...
export function NextHumanEventStudio({ foundingCircle }: { foundingCircle?: React.ReactNode }) {
  ...
  async function load() {
    setError("");
    const [eventResult, applicationResult, bookingResult] = await Promise.all([
    ...
    setEvent(eventResult.event); setStats(eventResult.stats); setPayments(eventResult.payments); setApplications(applicationResult.applications); setBookings(bookingResult.bookings);
  }

  useEffect(() => { void load().catch(caught => setError(caught instanceof Error ? caught.message : "NEXT HUMAN could not be loaded.")); }, []);
  ...
  async function decide(application: Application, status: string) {
    const internalNote = window.prompt("Internal review note", application.internalNote || "");
    if (internalNote === null) return;
    const scoreValue = window.prompt("Review score out of 100", String(application.score || 0));
    if (scoreValue === null) return;
    setBusy(application.id); setError("");
    try {
      const result = await eventApi<{ application: Application; message: string }>(`/applications/${application.id}`, { method: "PATCH", body: JSON.stringify({ status, internalNote, score: Number(scoreValue) }) });
      setApplications(current => current.map(item => item.id === application.id ? result.application : item)); setMessage(result.message);
  ...
  const tabs: { id: Tab; label: string }[] = [
    ...
    { id: "applications", label: "Applications" }, { id: "seating", label: "Daily Seats & Prices" }, { id: "bookings", label: "Day Bookings" }, { id: "founding", label: "Founding Circle" },
  ];
  ...
  return <div className="nh-admin">
    ...
    {tab === "applications" && <><div className="nh-admin-toolbar"><div><strong>Selection queue</strong><span>Approval unlocks day-by-day booking; it does not reserve any seat.</span></div><select value={applicationFilter} onChange={change => setApplicationFilter(change.target.value)}><option value="all">All applications</option><option value="submitted">Submitted</option><option value="under_review">Under review</option><option value="approved">Approved</option><option value="waitlisted">Waitlisted</option><option value="declined">Declined</option></select></div><div className="nh-admin-applications">{visibleApplications.length ? visibleApplications.map(application => <article key={application.id}><header><div><span className={`nh-status ${application.status}`}>{label(application.status)}</span><h2>{application.fullName}</h2><p>{application.memberNumber || "Member"} · {event.pathways[application.pathway].title} · {when(application.submittedAt)}</p></div><aside><b>{application.score}/100</b><a href={`mailto:${application.email}`}>{application.email}</a><a href={`tel:${application.mobile}`}>{application.mobile}</a></aside></header><ol>{application.answers.map((answer, index) => <li key={`${application.id}-${index}`}><b>{index + 1}</b><span><small>{event.pathways[application.pathway].questions[index]}</small>{answer}</span></li>)}</ol>{application.internalNote && <blockquote>Internal note: {application.internalNote}</blockquote>}<footer><button onClick={() => void decide(application, "under_review")} disabled={busy === application.id}>Review</button><button className="approve" onClick={() => void decide(application, "approved")} disabled={busy === application.id}>Approve</button><button onClick={() => void decide(application, "waitlisted")} disabled={busy === application.id}>Waitlist</button><button className="decline" onClick={() => void decide(application, "declined")} disabled={busy === application.id}>Decline</button></footer></article>) : <section className="nh-admin-empty"><strong>No applications in this view</strong><span>New Challenge and Fellowship submissions will appear here automatically.</span></section>}</div></>}
