...
import { CreativeStudioPanel, FocusCampaignsPanel } from "./campaign-studio";
...
type AdminView = "overview" | "next_human" | "members" | "certificates" | "yogdaan" | "sangha" | "sankalps" | "creative" | "campaigns" | "notifications";
...
export function AdminClient() {
  ...
  async function reviewNextHumanInquiry(item: NextHumanInquiry, status: string) {
    const internalNote = window.prompt("Internal note (optional)", item.internalNote || "");
  ...
  const navigation: { id: AdminView; label: string; badge: number | string }[] = [
    ...
    { id: "next_human", label: "NEXT HUMAN", badge: overview.metrics.newNextHumanInquiries },
    ...
    { id: "creative", label: "Creative Studio", badge: 4 }, { id: "campaigns", label: "Focus Campaigns", badge: "F" },
    ...
    <div className="admin-body"><aside><p>MASTER ADMINISTRATION</p>{navigation.map(item => <button key={item.id} className={view === item.id ? "active" : ""} onClick={() => setView(item.id)}><span>{item.badge}</span>{item.label}</button>)}</aside>
      <section className="admin-content">
        ...
        {view === "next_human" && <NextHumanEventStudio foundingCircle={!nextHumanInquiries.length ? <section className="admin-panel admin-nh-empty"><h2>No Founding Circle inquiry yet</h2><p>New foundation-team submissions from the public NEXT HUMAN page will appear here automatically.</p></section> : <div className="admin-nh-list">{nextHumanInquiries.map(item => <article key={item.id}>
          ...
          <div className="admin-nh-actions"><button className="admin-primary" disabled={busy === `next-human-${item.id}`} onClick={() => void reviewNextHumanInquiry(item, "orientation_invited")}>Invite to orientation</button><button className="admin-secondary" disabled={busy === `next-human-${item.id}`} onClick={() => void reviewNextHumanInquiry(item, "foundation_circle")}>Add to Founding Circle</button><button className="admin-secondary" disabled={busy === `next-human-${item.id}`} onClick={() => void reviewNextHumanInquiry(item, "reviewing")}>Mark reviewing</button><button className="admin-secondary" disabled={busy === `next-human-${item.id}`} onClick={() => void reviewNextHumanInquiry(item, "hold")}>Hold</button><button className="admin-secondary danger" disabled={busy === `next-human-${item.id}`} onClick={() => void reviewNextHumanInquiry(item, "declined")}>Close</button></div>
        </article>)}</div>} />}
        ...
        {view === "creative" && <CreativeStudioPanel />}
