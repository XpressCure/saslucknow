...
export function MemberClient() {
  ...
  return <main className={`member-shell member-portal${coordinatedCampaignClass}`}>
    <header className="member-header">
      ...
      <div className="member-account"><span>Namaste, {member.fullName.split(" ")[0]}</span>{["administrator", "super_administrator"].includes(member.role) && <Link className="member-admin-link" href="/admin">Administration</Link>}<button className="member-menu-toggle" type="button" aria-label={memberNavOpen ? "Close member menu" : "Open member menu"} aria-expanded={memberNavOpen} aria-controls="member-navigation" onClick={() => setMemberNavOpen(value => !value)}><span aria-hidden="true">{memberNavOpen ? "×" : "☰"}</span></button><button className="member-header-signout" onClick={signOut}>Sign out</button></div>
    </header>
    ...
    <nav id="member-navigation" className={`member-nav ${memberNavOpen ? "open" : ""}`} aria-label="Member portal" onClick={() => setMemberNavOpen(false)}>
      <div className="member-nav-profile"><div><strong>{member.fullName}</strong><span>Member</span><small>Sri Aurobindo Society</small></div><button type="button" aria-label="Close member menu" onClick={() => setMemberNavOpen(false)}>×</button></div>
      <Link className="member-nav-feature challenge" href="/bharat-uday?member=1"><span aria-hidden="true">✦</span><strong>The Next Human Challenge</strong><b aria-hidden="true">›</b></Link>
      <Link className="member-nav-feature conference" href="/next-human"><span aria-hidden="true">◌</span><strong>NEXT HUMAN 2026</strong><b aria-hidden="true">›</b></Link>
    ...
    <div className="member-content">
      ...
      {tab === "darshan" && <><div className={darshanPanel === "home" ? "member-dashboard-home" : ""}><PageHeading eyebrow="A SHARED FIELD OF WORK" title={`Namaste, ${member.fullName.split(" ")[0]}`} text="A quiet member space for study, meditation, conversation and inward discovery." />
        ...
        {dashboardCards.primaryFeature && nextHuman2026Live && <section className="member-dashboard-next-human-card"><p>COMING TO LUCKNOW</p><h2>NEXT HUMAN 2026</h2><span>A seven-evening inquiry into consciousness, evolution and the future human. Details are coming soon.</span><Link href="/next-human">Enter the experience <b>→</b></Link></section>}
    ...
    {!membershipDisabledMessage && <nav className="member-bottom-nav" aria-label="Primary member pages"><button type="button" className={tab === "darshan" ? "active" : ""} onClick={() => { setTab("darshan"); setDarshanPanel("home"); document.documentElement.scrollTop = 0; }}><span aria-hidden="true">⌂</span><small>Home</small></button><button type="button" className={tab === "sankalp" ? "active" : ""} onClick={() => { setTab("sankalp"); document.documentElement.scrollTop = 0; }}><span aria-hidden="true">◆</span><small>Sankalp</small></button><button type="button" className={tab === "yogdaan" ? "active" : ""} onClick={() => { setTab("yogdaan"); document.documentElement.scrollTop = 0; }}><span aria-hidden="true">₹</span><small>Yogdaan</small></button><button type="button" className={tab === "parichay" ? "active" : ""} onClick={() => { setTab("parichay"); document.documentElement.scrollTop = 0; }}><span aria-hidden="true">○</span><small>Parichay</small></button></nav>}
    {notice && <Notice notice={notice} close={() => setNotice(null)} />}
