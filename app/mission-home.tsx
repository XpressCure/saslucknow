...
type ContributionPaymentResult = {
...
function loadRazorpayCheckout() {
  return new Promise<void>((resolve, reject) => {
    ...
    if (existing) {
      ...
      existing.addEventListener("error", () => reject(new Error("The secure payment window could not load.")), { once: true });
      return;
    }
    ...
    script.onerror = () => reject(new Error("The secure payment window could not load. Please check your internet connection."));
    document.head.appendChild(script);
  });
}

async function contributionPaymentApi<T>(path: string, options: RequestInit = {}): Promise<T> {
...
const copy = {
  en: {
    nav: ["e-Library", "Events", "Join the Community"],
...
const savitriYoutubePlaylistId = "PLd_qJzN6LPeU";
const savitriYoutubePlaylistUrl = `https://www.youtube.com/playlist?list=${savitriYoutubePlaylistId}`;
const gatheringsYoutubePlaylistId = "PLJf4jD8jYb0A";
const gatheringsYoutubePlaylistUrl = `https://www.youtube.com/playlist?list=${gatheringsYoutubePlaylistId}`;
...
export function MissionHome() {
  ...
  const [, setContributionBusy] = useState(false);
  const [contributionResult, setContributionResult] = useState<ContributionPaymentResult | null>(null);
  const [contributionAccountEmail, setContributionAccountEmail] = useState("");
  const [contributionPrefill, setContributionPrefill] = useState({ name: "", email: "", mobile: "", amount: "" });
  ...
  const open = (name: typeof dialog) => {
    // Razorpay has been tested successfully, but public contributions are paused until launch.
    // Keep the completed payment flow below intact for straightforward reactivation later.
    setSent(name === "contribute");
    setUploading(false);
    setSubmitError("");
    setUploadReference("");
    setUploadStatus("pending");
    setContributionResult(null);
    setContributionAccountEmail("");
    setDialog(name);
  };
  useEffect(() => {
    const parameters = new URLSearchParams(window.location.search);
    setContributionPrefill({
      name: parameters.get("name") || "",
      email: parameters.get("email") || "",
      mobile: parameters.get("mobile") || "",
      amount: parameters.get("amount") || "",
    });
    if (parameters.get("contribute") === "1") {
      open("contribute");
      window.requestAnimationFrame(() => document.getElementById("support")?.scrollIntoView({ block: "start" }));
    }
  }, []);
  useEffect(() => {
    if (dialog !== "contribute") return;
    const timer = window.setTimeout(() => {
      const modal = document.querySelector<HTMLElement>(".modal");
      if (!modal) return;
      const values = contributionPrefill as Record<string, string>;
      for (const field of ["name", "email", "mobile", "amount"]) {
        const input = modal.querySelector<HTMLInputElement>(`input[name="${field}"]`);
        if (input && values[field]) input.value = values[field];
      }
    }, 0);
    return () => window.clearTimeout(timer);
  }, [dialog, contributionPrefill]);
  useEffect(() => {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (dialog === "contribute") {
      ...
      setSubmitError("");
      try {
        const orderResult = await contributionPaymentApi<{ order: { id: string; amountPaise: number; currency: string; razorpayKeyId: string; title: string } }>("/orders", {
          method: "POST",
          body: JSON.stringify({
            amountRupees: data.get("amount"), donorName: data.get("name"), donorEmail: data.get("email"),
            donorMobile: data.get("mobile"), city: data.get("city"), donorPan: data.get("donorPan"), donorAddress: data.get("donorAddress"),
          }),
        });
        ...
        const checkout = new Razorpay({
          ...
          prefill: { name: data.get("name"), email: data.get("email"), contact: data.get("mobile") },
          theme: { color: "#173846" },
          modal: { ondismiss: () => { setContributionBusy(false); setSubmitError("The payment window was closed. No contribution was recorded."); } },
          handler: async (payment: Record<string, string>) => {
            try {
              const verified = await contributionPaymentApi<ContributionPaymentResult>("/verify", {
                method: "POST",
              ...
              setContributionResult(verified);
              setContributionAccountEmail(String(data.get("email") || "").trim());
              setSent(true);
            } catch (error) {
              setSubmitError(error instanceof Error ? error.message : "The payment needs verification. Please contact the centre.");
            } finally {
        ...
        checkout.on("payment.failed", response => {
          setContributionBusy(false);
          setSubmitError(response.error?.description || "The payment was not completed. No contribution was recorded.");
        });
        checkout.open();
      } catch (error) {
        setSubmitError(error instanceof Error ? error.message : "Secure payment could not start.");
        setContributionBusy(false);
      }
      return;
    }
  ...
  const contributionSuccess = <div className="success contribution-payment-success">
    <span aria-hidden="true">&#10022;</span>
    <p className="kicker">SECURE CONTRIBUTIONS</p>
    <h2>Online contributions are coming soon.</h2>
    <p>Thank you for your wish to support the work. Our secure contribution facility has been tested successfully and will open shortly. No payment has been initiated.</p>
    <button className="button quiet" onClick={()=>setDialog(null)}>Close</button>
  </div>;
  ...
  return <div className={lang === "hi" ? "hindi" : ""}>
    ...
    <header className="site-header">
      ...
      <button className="header-donate" type="button" onClick={() => { setMenu(false); open("contribute"); }}>Donate Now</button>
    ...
    <main id="main">
      ...
      <section className="song-of-savitri gallery section" id="song-of-savitri" aria-labelledby="song-of-savitri-title">
        ...
        <div className={`gallery-track savitri-video-track ${savitriVideos.length ? "has-items" : ""}`} ref={savitriTrack} aria-label="The Song of Savitri videos" aria-live="polite">
          {savitriVideosLoading ? <div className="gallery-empty"><h3>Loading The Song of Savitri…</h3></div> : savitriVideos.length === 0 ? <div className="gallery-empty savitri-playlist-empty"><span>OFFICIAL SAS_LKO PLAYLIST</span><h3>The Song of Savitri</h3><p>The video series is available directly on our YouTube channel.</p><a className="button primary" href={savitriYoutubePlaylistUrl} target="_blank" rel="noreferrer">Watch the playlist on YouTube ↗</a></div> : savitriVideos.map(item => <article className="gallery-slide savitri-video-slide" key={item.id}><div className="gallery-media">{item.youtubeId ? <iframe title={`${item.cantoName}, lines ${item.lineNos}`} src={`https://www.youtube-nocookie.com/embed/${item.youtubeId}?rel=0&playsinline=1&list=${savitriYoutubePlaylistId}`} loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /> : <video controls preload="metadata" playsInline><source src={item.mediaUrl} type={item.mimeType}/>Your browser cannot play this video.</video>}</div><div className="gallery-caption"><span>Part {item.part}</span><h3><small>Book {item.bookNo} · Canto {item.cantoNo}</small>{item.cantoName}</h3><dl className="savitri-video-meta"><div><dt>Part</dt><dd>{item.part}</dd></div><div><dt>Book No.</dt><dd>{item.bookNo}</dd></div><div><dt>Canto No.</dt><dd>{item.cantoNo}</dd></div><div><dt>Name of Canto</dt><dd>{item.cantoName}</dd></div><div><dt>Line Nos.</dt><dd>{item.lineNos}</dd></div><div><dt>Page No.</dt><dd>{item.pageNo}</dd></div></dl></div></article>)}
        </div>
        <div className="gallery-action"><p>Watch every five-line visual passage in sequence, or add the next passage with its complete Savitri reference.</p><div className="gallery-action-buttons"><a className="button quiet" href={savitriYoutubePlaylistUrl} target="_blank" rel="noreferrer">View full YouTube playlist ↗</a><button className="button quiet" onClick={()=>open("savitri")}>Add a Song of Savitri video →</button></div></div>
      </section>
...
<section className="gallery section" id="gallery"><div className="section-title gallery-heading"><div><p className="kicker">MEMORIES OF THE WORK</p><h2>Gatherings through the years</h2></div><p>A growing visual record of lectures, study circles, observances, shrine visits and community moments.</p></div>{galleryItems.length > 0 && <div className="gallery-slider-header"><div className="gallery-controls" aria-label="Gallery navigation"><button type="button" onClick={()=>moveGallery(-1)} aria-label="Previous gallery items">←</button><button type="button" onClick={()=>moveGallery(1)} aria-label="Next gallery items">→</button></div></div>}<div className={`gallery-track ${galleryItems.length ? "has-items" : ""}`} ref={galleryTrack} aria-label="Event photographs and videos" aria-live="polite">{galleryLoading ? <div className="gallery-empty">Loading event memories…</div> : galleryItems.length === 0 ? <div className="gallery-empty savitri-playlist-empty"><span>OFFICIAL SAS_LKO PLAYLIST</span><h3>Collective Learnings</h3><p>Recorded gatherings are available directly on our YouTube channel.</p><a className="button primary" href={gatheringsYoutubePlaylistUrl} target="_blank" rel="noreferrer">Watch the playlist on YouTube ↗</a></div> : galleryItems.map(item => <article className="gallery-slide" key={item.id}><div className="gallery-media">{item.youtubeId ? <iframe title={item.title} src={`https://www.youtube-nocookie.com/embed/${item.youtubeId}?rel=0&playsinline=1&list=${gatheringsYoutubePlaylistId}`} loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /> : item.kind === "video" ? <video controls preload="metadata" playsInline><source src={item.mediaUrl} type={item.mimeType}/>Your browser cannot play this video.</video> : <img src={item.mediaUrl} alt={`${item.title} event photograph`} loading="lazy"/>}</div><div className="gallery-caption"><span>{item.category}</span><h3><small>Event</small>{item.title}</h3><p>{item.description}</p><time dateTime={item.eventDate}>{displayDate(item.eventDate)}</time></div></article>)}</div><div className="gallery-action"><p>Watch the complete Collective Learnings archive, or submit an event photograph or YouTube video with its details.</p><div className="gallery-action-buttons"><a className="button quiet" href={gatheringsYoutubePlaylistUrl} target="_blank" rel="noreferrer">View full YouTube playlist ↗</a><button className="button quiet" onClick={()=>open("gallery")}>Add event photos or videos →</button></div></div></section>
      ...
      <section className="community section" id="community"><p className="kicker">A SHARED JOURNEY</p><h2>{t.community}</h2><div className="community-grid"><a href="/joincommunity#parichay"><span>01</span><b>{t.join}</b><small>Share your Parichay and enter the member space →</small></a><a href="/member"><span>02</span><b>Member Login</b><small>Already registered? Continue to your member space →</small></a></div></section>

      <section className="support section" id="support"><div><p className="kicker">A QUIET INVITATION</p><h2>{t.support}</h2><p>{t.supportText}</p></div><button className="button quiet light" onClick={()=>open("contribute")}>{t.contribute} →</button></section>
    </main>
