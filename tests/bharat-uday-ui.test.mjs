...
test("the film holds its closing message while the real CTA stays outside it", async () => {
  ...
  assert.match(client, /Level 1 opened on/);
...
test("the launch challenge uses ten fixed mixed questions and reveals feedback", async () => {
  ...
  assert.match(client, /questionOrderFor\(nextAttempt, bharatUdayLevels\[0\]\.discoveries\.length\)/);
  assert.match(client, /You will see the correct answer immediately/);
  assert.match(client, /Not quite\. The correct answer is/);
  assert.match(client, /Array\.from\(\{ length: questionOrder\.length \}/);
  assert.match(client, /out of 100/);
  assert.match(client, /No qualifying benchmark/);
  assert.match(data, /function questionOrderFor/);
  assert.match(data, /const levelOneLaunchDiscoveries/);
  assert.match(data, /Mangalyaan orbited which planet/);
  assert.match(css, /\.bu-question-progress\{display:grid;grid-template-columns:repeat\(10,1fr\)/);
  assert.match(css, /\.bu-question-progress span\.current/);
});

test("only Level 1 is visible and live on 31 August 2026", async () => {
  const client = await read("../app/bharat-uday/bharat-uday-client.tsx");
  assert.match(client, /const launchDate = "31 August 2026"/);
  assert.match(client, /filter\(item => item\.number === launchLevel\)/);
  assert.match(client, /Only Level 1 is open/);
  assert.doesNotMatch(client, /Opening date to be announced/);
  assert.match(client, /More levels coming soon/);
});
