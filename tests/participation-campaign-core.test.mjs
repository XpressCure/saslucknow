...
test("SAS Creative Studio exposes the approved spiritual and discovery templates", () => {
  ...
  assert.deepEqual(catalog.map(item => item.id), ["inner_silence", "song_of_savitri", "collective_sadhana", "mothers_guidance", "bharat_uday", "golden_aspiration", "vedic_dawn", "lotus_path", "evolutionary_fire", "matrimandir_light"]);
...
test("creative validation fixes its theme and destination to the approved template", () => {
  ...
  const result = validateCreativeInput({
    ...
    dashboardCards: { primaryFeature: false, nextHumanChallenge: true, latestSangha: false, meaningfulAction: true, consciousOffering: false },
    dashboardShortcuts: { sangha: true, innerRoom: false, sankalp: true, yogdaan: false, reflections: true },
    pageVisibility: { nextHuman2026: false, nextHumanChallenge: true },
    copy: template.copy,
  });
  assert.equal(result.ok, true);
  assert.equal(result.value.destination, "watch");
  assert.equal(result.value.themePackId, "indigo_savitri");
  assert.deepEqual(result.value.dashboardCards, { primaryFeature: false, nextHumanChallenge: true, latestSangha: false, meaningfulAction: true, consciousOffering: false });
  assert.deepEqual(result.value.dashboardShortcuts, { sangha: true, innerRoom: false, sankalp: true, yogdaan: false, reflections: true });
  assert.deepEqual(result.value.pageVisibility, { nextHuman2026: false, nextHumanChallenge: true });
});
...
test("Creative Studio exposes only approved member-app destinations", () => {
  ...
  assert.deepEqual(destinations.map(item => item.id), ["dashboard", "inner-room", "reflections", "sound", "sangha", "watch", "library", "sakhi", "sankalp", "yogdaan", "parichay", "next-human", "bharat-uday"]);
});
...
test("member campaign view contains display fields but not administrator audit data", () => {
  ...
  assert.deepEqual(view.dashboardCards, { primaryFeature: true, nextHumanChallenge: true, latestSangha: true, meaningfulAction: true, consciousOffering: true });
  assert.deepEqual(view.dashboardShortcuts, { sangha: true, innerRoom: true, sankalp: true, yogdaan: true, reflections: true });
  assert.deepEqual(view.pageVisibility, { nextHuman2026: true, nextHumanChallenge: true });
